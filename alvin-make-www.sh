...
# -----------------------------------------------------------------------------
# add ipv6 dns, to prevent Bonjour lookup service
# see: https://stackoverflow.com/a/45331805/6049731
# -----------------------------------------------------------------------------
# check if dns already declared in the hosts file or not
if grep -Fxq "::1         ${MY_HOSTNAME} # generated by alvin-make-www" /etc/hosts
then
    # code if found
    echo "FAILED: added dns ${MY_HOSTNAME} to the hosts file"
    echo "------- dns ipv6 already exist in hosts file"
else
    # if not found, add dns to the host file
    echo "::1         ${MY_HOSTNAME} # generated by alvin-make-www" | sudo tee -a /etc/hosts > /dev/null
    echo "done: add dns ipv6 ${MY_HOSTNAME} to the hosts file"
fi
